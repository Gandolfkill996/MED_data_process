...
def get_vistis_intervals():
    ...
    visits_record = read_xlsx(address,"5_Visit Dates_20260304")
    ...
    for i in range(len(visits_record)):
        ...
        # In 5_Visit Dates_20260304 table, some records' month missing unregular visit which is not included in calculation
...
def calculation(current_date, cohort_type):
    ...
    for ID in visits_ini_off.keys():
        # If there is not visit record in 5_Visit Dates_20260304
...
def to_excel(month_count, cohort_type):
    ...
    if cohort_type == 'origin':
        sheet_name = "2_Orig Cohort Sample Output 20260304"
    elif cohort_type == 'new':
        sheet_name = "3_New Cohort Sample Output 20260304"
    else:
...
def judge_visit_window_condition():
    ...
    print(visits_record)
    with pd.ExcelWriter(address, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
        visits_record.to_excel(writer, sheet_name="5_Visit Dates_20260304", index=False)
...
if __name__ == "__main__":
    ...
    for c in ['origin', 'new']:
        month_count, cohort = count_output("03/04/2026", c)
